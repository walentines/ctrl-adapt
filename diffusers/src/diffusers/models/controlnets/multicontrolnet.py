...
class MultiControlNetModel(ModelMixin):
    ...
    def __init__(self, controlnets: Union[List[ControlNetModel], Tuple[ControlNetModel]], fusion_model: nn.Module, fusion_model_mid: nn.Module):
        super().__init__()
        self.nets = nn.ModuleList(controlnets)
        self.fusion_model = fusion_model
        self.fusion_model_mid = fusion_model_mid
    ...
    def forward(
        ...
        down_samples_seg, mid_sample_seg = self.nets[0](
            sample=sample,
            timestep=timestep,
        ...
        down_samples_depth, mid_sample_depth = self.nets[1](
            sample=sample,
            timestep=timestep,
            encoder_hidden_states=encoder_hidden_states,
            controlnet_cond=controlnet_cond[1],
            conditioning_scale=conditioning_scale[1],
            class_labels=class_labels,
            timestep_cond=timestep_cond,
            attention_mask=attention_mask,
            added_cond_kwargs=added_cond_kwargs,
        ...
        down_block_additional_residuals = self.fusion_model(down_samples_seg, down_samples_depth)
        mid_block_additional_residual = self.fusion_model_mid([mid_sample_seg], [mid_sample_depth])[0]  # Mid block is single
