...
def make_dataset(config, mode):
    ...
    if mode == 'training':
        ...
        dataset = CityscapesDataset(config.dataloader.training.segmentation_path,config.dataloader.training.depth_path, config.dataloader.training.image_path)
    elif mode == 'validation':
        dataset = CityscapesDataset(config.dataloader.validation.segmentation_path,config.dataloader.validation.depth_path, config.dataloader.validation.image_path)
    else:
...
def instantiate_and_configure_models(config, accelerator, logger):
    ...
    return vae, unet, text_encoder, controlnet, noise_scheduler, weight_dtype
